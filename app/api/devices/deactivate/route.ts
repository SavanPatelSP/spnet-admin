...
export async function POST(req: Request) {
  try {
    ...
    if (activation.status === "INACTIVE") {
      return Response.json({ success: true, message: "Device is already inactive" });
    }
    ...
    await logAudit(
      ...
      session.user.role,
      session.user.name,
      `Deactivated device ${activation.deviceName || activation.deviceId}`
    );

    return Response.json({ success: true, data: { id: body.id, status: "INACTIVE" } });
  } catch (error) {
