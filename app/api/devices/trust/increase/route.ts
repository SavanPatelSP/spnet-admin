...
export async function POST(req: Request) {
  try {
    ...
    const newTrustScore = Math.min(100, activation.trustScore + 10);
    ...
    await logAudit(
      ...
      session.user.role,
      session.user.name,
      `Increased trust score from ${activation.trustScore} to ${newTrustScore} for device ${activation.deviceName || activation.deviceId}`
    );
