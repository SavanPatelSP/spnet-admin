...
export function AuthFooter() {
  ...
  return (
    <footer className="border-t border-zinc-800/20 bg-zinc-950/60 px-6 py-3 backdrop-blur-sm">
      <div className="flex flex-col items-center justify-between gap-1.5 sm:flex-row">
        ...
        <div className="flex items-center gap-2">
          <span className="text-[10px] text-zinc-600">{APP_NAME} Administration Platform</span>
          <span className="text-zinc-700/40">|</span>
          <div className="flex items-center gap-1">
            <span className={`h-1 w-1 rounded-full ${envColor}`} />
            <span className="text-[9px] text-zinc-600">{envLabel}</span>
          </div>
        </div>
      </div>
    </footer>
