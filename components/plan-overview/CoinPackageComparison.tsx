        ...
        >
          {packages.map((p) => (
            <option key={p.label} value={p.label}>
              {p.label} — {fmt(p.amount)} coins at {p.currency}{p.price.toLocaleString()}
            </option>
          ))}
        </select>
        ...
        <div className="flex items-center gap-3 text-xs text-zinc-400">
          ...
          <span className="text-zinc-500">
            {second.amount > first.amount
              ? `${fmt(second.amount - first.amount)} additional coins for ${second.currency}${(second.price - first.price).toLocaleString()} more`
              : `${fmt(first.amount - second.amount)} fewer coins, saving ${second.currency}${(first.price - second.price).toLocaleString()}`}
          </span>
