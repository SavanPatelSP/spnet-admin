...
export default function SetGemsModal({ licenseId, organization, currentBalance, autoOpen }: SetGemsModalProps) {
  ...
  async function handleSet() {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/gems/set", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ licenseId, balance, type, reason: reason || null }),
      });
      const data = await res.json();
      if (!res.ok) { setError(data.error || "Failed to set gem balance"); return; }
      setOpen(false);
      router.refresh();
    } catch { setError("Failed to set gem balance"); }
  ...
  return (
    <>
      ...
      <Modal
        ...
        title="Set Gem Balance"
        description={`Set exact gem balance for ${organization}.`}
        size="lg"
        footer={
          <>
      ...
      >
        <div className="space-y-6">
          ...
          <div className="rounded-xl border border-blue-500/20 bg-blue-500/5 p-5">
            ...
            <div>
              <label className="mb-1.5 block text-xs font-medium text-zinc-400">Gem Type</label>
              <select value={type} onChange={(e) => setType(e.target.value)}
                className="w-full rounded-xl border border-zinc-700 bg-zinc-800 p-2.5 text-sm text-zinc-100 outline-none focus:border-blue-500">
                {["FINITE", "PROMOTIONAL", "REWARD"].map((t) => <option key={t} value={t}>{t}</option>)}
              </select>
            </div>
          </div>
            ...
            <div className="mt-4 grid grid-cols-2 gap-3">
              <div className={`rounded-lg border p-2.5 ${
                ...
                <div className="flex items-center gap-1.5 mb-1">
                  <Crown size={11} className={premiumEligible ? "text-yellow-400" : "text-zinc-500"} />
                  <span className={`text-[10px] font-medium ${premiumEligible ? "text-yellow-400" : "text-zinc-500"}`}>
                    Premium Power
                  </span>
                </div>
                <span className={`text-[10px] ${premiumEligible ? "text-yellow-200/70" : "text-zinc-500"}`}>
                  {premiumEligible ? "Eligible" : wasPremiumEligible && !premiumEligible
                    ? "Lost eligibility" : "Insufficient"}
                </span>
              </div>
              <div className={`rounded-lg border p-2.5 ${
                ...
                <div className="flex items-center gap-1.5 mb-1">
                  <Shield size={11} className={licenseEligible ? "text-blue-400" : "text-zinc-500"} />
                  <span className={`text-[10px] font-medium ${licenseEligible ? "text-blue-400" : "text-zinc-500"}`}>
                    License Power
                  </span>
                </div>
                <span className={`text-[10px] ${licenseEligible ? "text-blue-200/70" : "text-zinc-500"}`}>
                  {licenseEligible ? "Eligible" : wasLicenseEligible && !licenseEligible
                    ? "Lost eligibility" : "Insufficient"}
                </span>
              </div>
            </div>
          ...
          <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-4">
            ...
            <div className="space-y-1 font-mono text-xs">
              <div className="flex">
                ...
                <span className="text-yellow-400">GEMS_SET</span>
              </div>
              <div className="flex">
              ...
              <div className="flex">
                ...
                <span className={premiumEligible ? "text-yellow-400" : "text-zinc-600"}>
                  {premiumEligible ? "Eligible" : wasPremiumEligible ? "Lost" : "\u2014"}
                </span>
              </div>
              <div className="flex">
                <span className="w-28 text-zinc-500">License Power</span>
                <span className={licenseEligible ? "text-blue-400" : "text-zinc-600"}>
                  {licenseEligible ? "Eligible" : wasLicenseEligible ? "Lost" : "\u2014"}
                </span>
              </div>
              <div className="flex">
