...
export default function SetGemsModal({ licenseId, organization, currentBalance }: SetGemsModalProps) {
  ...
  async function handleSet() {
    ...
    try {
      ...
      if (!res.ok) {
        setError(data.error || "Failed to set gem balance");
        return;
      }
      setOpen(false);
      router.refresh();
    } catch {
      setError("Failed to set gem balance");
    } finally {
      setLoading(false);
    }
  }
    ...
    <>
      ...
      >
        <div className="space-y-4">
          ...
          <div>
            <label className="mb-1.5 block text-sm font-medium text-zinc-400">Gem Type</label>
