...
import RefundCoinsButton from "@/components/coins/RefundCoinsButton";
...
const TX_TYPES = ["CREDIT", "DEBIT", "REFUND", "ADJUSTMENT"];
...
export function CoinHistoryTable({ transactions }: { transactions: TransactionRow[] }) {
  ...
  function handleExportCSV() {
    ...
    downloadCSV("coin-transactions", headers, rows);
  }
    ...
    <DataTable
      ...
      rows={filtered.map((t) => ({
        ...
        cells: [
          ...
          <span key="type" className={`inline-block rounded-full border px-2.5 py-0.5 text-xs font-medium ${typeColors[t.type] || "bg-zinc-500/10 text-zinc-400 border-zinc-500/20"}`}>
            {t.type}
          </span>,
          ...
          <div key="actions" className="flex justify-end">
            <RefundCoinsButton transactionId={t.id} type={t.type} />
          </div>,
        ],
      }))}
      pageSize={15}
