...
export function CoinDistributionChart({ balances }: { balances: BalanceEntry[] }) {
  const stats = useMemo(() => {
  ...
  return (
    <div className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-6">
      ...
      {stats.total === 0 ? (
        <p className="text-sm text-zinc-500">No coin data available.</p>
      ) : (
        <div className="grid grid-cols-2 gap-4">
          <div className="rounded-lg bg-zinc-800/50 p-3">
            <p className="text-xs text-zinc-500">Total Supply</p>
            <p className="mt-1 text-lg font-bold text-yellow-400">{stats.total.toLocaleString()}</p>
          </div>
          <div className="rounded-lg bg-zinc-800/50 p-3">
